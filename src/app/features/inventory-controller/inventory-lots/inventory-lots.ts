import { Component, OnInit, signal, computed } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { InventoryControllerService } from '../../../core/services/inventory-controller.service';
import { InventoryLot } from '../../../core/models/inventory-controller.model';

@Component({
  selector: 'app-inventory-lots',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="space-y-5 p-6">
      <!-- Header -->
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 class="text-xl font-bold text-gray-800">Inventory Lots</h2>
          <p class="text-gray-500 text-sm mt-0.5">Track all lot records with batch and expiry details</p>
        </div>
        <button
          (click)="loadData()"
          class="bg-teal-600 text-white px-3 py-2 rounded-lg text-sm hover:bg-teal-700 transition-colors font-medium flex items-center gap-2"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
          Refresh
        </button>
      </div>

      <!-- Filters -->
      <div class="bg-white rounded-xl border border-gray-200 px-4 py-3 flex flex-col sm:flex-row gap-3">
        <div class="flex-1 relative">
          <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <input
            type="text"
            [value]="searchTerm()"
            (input)="searchTerm.set(getInputValue($event))"
            placeholder="Search by item name or batch number..."
            class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500"
          />
        </div>
        <select
          [value]="statusFilter()"
          (change)="statusFilter.set(getInputValue($event))"
          class="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white"
        >
          <option value="">All statuses</option>
          <option value="1">Active</option>
          <option value="2">Quarantined</option>
          <option value="3">Expired</option>
          <option value="4">Disposed</option>
        </select>
      </div>

      @if (loadError()) {
        <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
          {{ loadError() }}
        </div>
      }

      <!-- Summary -->
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="bg-green-50 border border-green-200 rounded-xl p-3 text-center">
          <p class="text-xs font-semibold text-green-600 uppercase">Active</p>
          <p class="text-xl font-bold text-green-700 mt-1">{{ countByStatus(1) }}</p>
        </div>
        <div class="bg-orange-50 border border-orange-200 rounded-xl p-3 text-center">
          <p class="text-xs font-semibold text-orange-600 uppercase">Quarantined</p>
          <p class="text-xl font-bold text-orange-700 mt-1">{{ countByStatus(2) }}</p>
        </div>
        <div class="bg-red-50 border border-red-200 rounded-xl p-3 text-center">
          <p class="text-xs font-semibold text-red-600 uppercase">Expired</p>
          <p class="text-xl font-bold text-red-700 mt-1">{{ countByStatus(3) }}</p>
        </div>
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center">
          <p class="text-xs font-semibold text-gray-500 uppercase">Total</p>
          <p class="text-xl font-bold text-gray-700 mt-1">{{ lots().length }}</p>
        </div>
      </div>

      <!-- Table -->
      <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        @if (loading()) {
          <div class="flex items-center justify-center py-12">
            <div class="w-7 h-7 border-4 border-teal-600 border-t-transparent rounded-full animate-spin"></div>
          </div>
        } @else if (filteredLots().length === 0) {
          <div class="text-center py-12">
            <svg class="w-10 h-10 text-gray-300 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
            </svg>
            <p class="text-gray-500 font-medium text-sm">No inventory lots found</p>
            <p class="text-gray-400 text-xs mt-1">Try adjusting your filters</p>
          </div>
        } @else {
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Lot ID</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Item</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Batch #</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Expiry Date</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                  <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Days Left</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                @for (lot of filteredLots(); track lot.inventoryLotId) {
                  <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 text-gray-400 text-xs">#{{ lot.inventoryLotId }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ lot.itemName ?? lot.itemId }}</td>
                    <td class="px-4 py-3 font-medium text-gray-800">#{{ lot.batchNumber }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ lot.expiryDate | date:'dd MMM yyyy' }}</td>
                    <td class="px-4 py-3">
                      <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold"
                        [class]="getLotStatusClass(lot.status)">
                        {{ getLotStatusLabel(lot.status) }}
                      </span>
                    </td>
                    <td class="px-4 py-3">
                      <span class="text-sm font-bold" [class]="getDaysClass(lot.expiryDate)">
                        {{ getDaysLeft(lot.expiryDate) }}
                      </span>
                    </td>
                  </tr>
                }
              </tbody>
            </table>
          </div>
          <div class="px-4 py-2.5 border-t border-gray-100 text-xs text-gray-400">
            {{ filteredLots().length }} of {{ lots().length }} lots
          </div>
        }
      </div>
    </div>
  `,
})
export class InventoryLotsComponent implements OnInit {
  lots = signal<InventoryLot[]>([]);
  loading = signal(true);
  loadError = signal('');
  searchTerm = signal('');
  statusFilter = signal('');

  filteredLots = computed(() => {
    const term = this.searchTerm().toLowerCase().trim();
    const status = this.statusFilter();
    return this.lots().filter((l) => {
      const matchesTerm =
        !term ||
        l.itemName?.toLowerCase().includes(term) ||
        l.batchNumber?.toString().toLowerCase().includes(term) ||
        l.itemId?.toString().includes(term);
      const matchesStatus = !status || l.status.toString() === status;
      return matchesTerm && matchesStatus;
    });
  });

  countByStatus = (s: number) => this.lots().filter((l) => l.status === s).length;

  constructor(private icService: InventoryControllerService) {}

  ngOnInit() { this.loadData(); }

  loadData() {
    this.loading.set(true);
    this.loadError.set('');
    this.icService.getInventoryLots().subscribe({
      next: (data) => { this.lots.set(data); this.loading.set(false); },
      error: (err) => {
        this.loadError.set(`Failed to load lots (${err.status}: ${err.error?.message || err.message || 'Unknown error'})`);
        this.loading.set(false);
      },
    });
  }

  getInputValue(event: Event): string {
    return (event.target as HTMLInputElement | HTMLSelectElement).value;
  }

  getLotStatusClass(status: number): string {
    switch (status) {
      case 1: return 'bg-green-100 text-green-700';
      case 2: return 'bg-orange-100 text-orange-700';
      case 3: return 'bg-red-100 text-red-700';
      case 4: return 'bg-gray-100 text-gray-500';
      default: return 'bg-gray-100 text-gray-500';
    }
  }

  getLotStatusLabel(status: number): string {
    switch (status) {
      case 1: return 'Active';
      case 2: return 'Quarantined';
      case 3: return 'Expired';
      case 4: return 'Disposed';
      default: return 'Unknown';
    }
  }

  getDaysLeft(expiryDateStr: string): string {
    const expiry = new Date(expiryDateStr);
    const today = new Date();
    const diff = Math.floor((expiry.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    if (diff < 0) return 'Expired';
    if (diff === 0) return 'Today';
    return `${diff}d`;
  }

  getDaysClass(expiryDateStr: string): string {
    const expiry = new Date(expiryDateStr);
    const today = new Date();
    const diff = Math.floor((expiry.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    if (diff < 0) return 'text-red-600';
    if (diff <= 30) return 'text-amber-600';
    if (diff <= 90) return 'text-yellow-600';
    return 'text-green-600';
  }
}
